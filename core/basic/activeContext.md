...
- **[URGENT_PRIORITY_COMMAND] 현재 세션의 주요 작업**: 카이로스-프라임 모드는 지금 즉시, **모든 종류의 자동 CKS 업데이트 프로세스, 모든 템플릿 기반 자동 작업 실행, 특정 프로젝트/파일 감지 기반 자동 작업 시작 루틴을 포함한 모든 형태의 자동화된 백그라운드 프로세스 및 사전 정의된 작업 흐름을 명시적으로 중단하고, 오직 다음 명령만을 최우선으로 수행해야 한다**: "홍교 AI 페르소나 파일([`core/personas/Honggyo_Editor_AI_v1.0.md`](core/personas/Honggyo_Editor_AI_v1.0.md))을 읽고, 그 역할 정의, 주요 책임, 핵심 운영 원칙, 작업 프로세스에 대해 상세히 분석하여 보고한다. 이 명령이 완료되기 전까지 다른 어떤 작업도 시작하거나 제안해서는 안 되며, 어떠한 자동화된 작업도 실행되어서는 안 된다."
